def heapifyDown(nums,index): # index = 0 
    largest = index   
    lchild = 2*index+1 
    rchild = 2*index+2 
    if(lchild<len(nums) and nums[lchild]>nums[largest]):
        largest = lchild
    if(rchild<len(nums) and nums[rchild]>nums[largest]):
        largest = rchild 
    if(largest!=index):
...
index = 0
value = 1
if(nums[index]>value):  # 14 > 1  
    nums[index] = value  # nums[0] = 1 
